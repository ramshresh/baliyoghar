<?php 
	...
	class Nepali_Calendar {
		...
		public function eng_to_nep($bcipn_paramdate){
                    
                    /* modified for bcipn - gaurab- */
                    $bcipndate = explode("-",$bcipn_paramdate);
                    $yy = $bcipndate[0];
                    $mm = $bcipndate[1];
                    $dd = $bcipndate[2];
                    /* end modification */
			...
			if ($this->is_range_eng($yy, $mm, $dd) == false){
				...
				$this->nep_date["month"] = $m;
                                $this->bcipndate = $y.'-'.$m.'-'.$total_nDays;
		 ...
		 * @return unknown
		 */
		public function nep_to_eng($bcipn_paramdate){
			...
			if($this->is_range_nep($yy, $mm, $dd)===false){
				...
				$numDay = $day;
                                ...
                                $this->bcipndate = $y.'-'.$m.'-'.$total_eDays;
