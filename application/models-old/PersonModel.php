...
class personmodel extends CI_Model {
    ...
    function do_upload($person_id) {
        ...
        if (strtolower($image_data['file_ext']) == '.jpg' || strtolower($image_data['file_ext']) == '.jpeg' || strtolower($image_data['file_ext']) == '.png' || strtolower($image_data['file_ext']) == '.gif') {
            ...
            $success = $this->db->update('person', array('photo' => 'image_' . $person_id . "" . $image_data['file_ext']));
        }
    }
    ...
    public function getPerson($search_string, $start, $end) {
        ...
        if ($this->session->userdata('role') == 'superadmin') {
            ...
            $query = $this->db->query("SELECT * FROM person WHERE (fullname like '%" . mysql_real_escape_string($search_string) . "%' OR email like '%" . mysql_real_escape_string($search_string) . "%' or mobile like '%" . mysql_real_escape_string($search_string) . "%') AND deleted=0  ORDER BY fullname ASC LIMIT " . $start . " , " . $end);
        }
