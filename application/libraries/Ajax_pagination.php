...
class Ajax_pagination{
    ...
    function create_links(){
        ...
        $per_page_list=array(10,20,50,100,500);
        $optionsPerPage='<option value="">All</option>';
        foreach ($per_page_list as $i){
            $optionsPerPage.='<option value="'.$i.'"  >'.$i.'</option>';
        }

        // SHOWING LINKS
        if ($this->show_count){
            $curr_offset = $CI->uri->segment($this->uri_segment);
            $info = 'Show All: <input name="show_all" type="checkbox"/> | Number of rows: <select name="per_page">'.$optionsPerPage.'</select> | Showing ' . ( $curr_offset + 1 ) . ' to ' ;
