<?php echo form_open_multipart('Person/editParticipationSave_async', array('id' => 'edit_event_participation_form', 'name' => 'edit_event_participation_form')); ?>
<input type="hidden" name="person_id" value="<?= $person_id ?>">
<input type="hidden" name="event_id" value="<?= $event_id ?>">
<input type="hidden" name="participated_in_id" value="<?= $participated_in_id ?>">
<input type="hidden" name="i" value="<?= $i ?>">
<h2><strong>Edit Participation Details:</strong></h2><hr/>
...
<script>
	var root = '../';
	// process the form
	$('form#edit_event_participation_form').submit(function (event) {
		event.preventDefault();
		// get the form data
		// there are many ways to get this data using jQuery (you can use the class or id also)
		var $form = $('form#edit_event_participation_form');
...
// "person_email":"ramshrestha@nset.org.np","person_org_name":"NSET","person_org_address":"Sainbu-4, Bhaisepati, Lalitpur","person_org_phone":"01 551 0000","person_org_fax":"31231","person_position":"Geomatics Engineer","person_current_status":"Active","education":"","dropdownindex":"","work_type_id":"2"}
		...
		$.ajax({
			...
			.done(function (data) {
				...
				$('tr#event_row_' + person_id).html(data);

				$("#participationFormDiv").dialog('close');
